...
export const FALLBACK_IMAGE_POOL: Record<string, string[]> = {
  world: [
...
const appendQueryParam = (url: string, key: string, value: string | number): string => {
...
export const upgradeNewsImageUrl = (
...
const DEFAULT_FALLBACK_IMAGES = FALLBACK_IMAGE_POOL['default'];

const getFallbackPool = (categorySlug: string): string[] =>
  FALLBACK_IMAGE_POOL[categorySlug] ?? DEFAULT_FALLBACK_IMAGES;

export const getArticleFallbackImage = (
  categorySlug = 'default',
  key = '',
  targetWidth = DEFAULT_WIDTH
): string => {
  const pool = getFallbackPool(categorySlug);
  const index = hashString(key || categorySlug) % pool.length;
  return upgradeNewsImageUrl(pool[index], targetWidth);
};
...
export const resolveNewsImageUrl = (
  ...
  const upgraded = upgradeNewsImageUrl(url, targetWidth);
  if (upgraded) {
    return upgraded;
  }

  return getArticleFallbackImage(categorySlug, key, targetWidth);
};

export const newsImageSrcSet = (
  url: string | null | undefined,
  widths: number[] = [640, 960, 1200],
  categorySlug = 'default',
  key = ''
): string => {
  const base = resolveNewsImageUrl(url, categorySlug, key, widths[widths.length - 1]);
  if (!base) {
    return '';
  }

  return widths
    .map((width) => `${upgradeNewsImageUrl(base, width)} ${width}w`)
    .join(', ');
};
